...
class RobotDataProcessor:
    ...
    def preprocess_data(self) -> pd.DataFrame:
        ...
        # Ignoramos la diferencia entre greedy y selective, unificamos bajo 'caso'
    ...
    def plot_comparison_histograms(self, save_dir: Optional[str] = None) -> None:
        """Plot histograms for all cases overlaid for comparison."""
    ...
    def plot_performance_boxplot(self, window_sec: int = 300, max_time_sec: int = 3000, save_path: Optional[str] = None) -> None:
        """Generates a boxplot showing tasks completed per time window for all cases, with connected medians."""
        if self.raw_data is None or self.raw_data.empty: return

        logger.info("Generating performance boxplot with trend lines for cases")
        df = self.raw_data.copy()

        # Filtering and make tasks per time window
        ...
        # Grouping ignoring strategies, using cases
        ...
        # ---> AÑADIR LÍNEAS DE TENDENCIA (MEDIANAS) <---
        medians = perf.groupby(['caso', 'time_window'])['tasks_completed'].median().reset_index()
    ...
    def plot_search_time_distribution(self, max_time_sec: int = 3000, save_path: Optional[str] = None) -> None:
        """Generates a violin plot to show the overall distribution of task search times per case."""
        if self.raw_data is None or self.raw_data.empty: return
    ...
    def plot_f_measure_boxplot(self, window_sec: int = 300, max_time_sec: int = 3000, save_path: Optional[str] = None) -> None:
        """Generates a boxplot of the F-measure over time, comparing cases."""
        if self.raw_data is None or self.raw_data.empty: return
        ...
        # ---> LÍNEAS DE TENDENCIA PARA F-MEASURE <---
...
class SpecializationScatterPlotter:
    ...
    def preprocess_data(self) -> pd.DataFrame:
        ...
        # Agrupamos por caso en vez de estrategia
        df['experiment_id'] = df.groupby(['caso', 'seed']).ngroup()
    ...
    def plot_figure(self, save_path: str) -> None:
        """Plots all cases in the same figure differentiated by color"""
...
def main():
    ...
    output_dir = r"/home/gmadro/EXP_CASOS/processing_data"
    
    try:
        # Carga masiva de todos los csv de los casos
        ...
        print("\n" + "="*50)
        print("GENERATING PLOTS & FIGURES (CASE COMPARISON)")
        print("="*50)

        # 1. Histogramas
        processor.plot_comparison_histograms(save_dir=output_dir)
            
        # 2. Performance Boxplot (con las líneas conectando las medianas)
        processor.plot_performance_boxplot(
            window_sec=300,
            max_time_sec=3000,
            save_path=f"{output_dir}/figure6_performance_boxplot_cases.png"
        )
