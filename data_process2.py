...
class RobotDataProcessor:
    ...
    def plot_performance_trend(self, window_sec: int = 300, max_time_sec: int = 10000, save_path: Optional[str] = None) -> None:
        ...
        # Filtrado temporal y asignación de ventanas
        ...
        # NUEVO: Gráfica de líneas continua con estimación y sombreado de desviación estándar nativo
        sns.lineplot(
            data=perf,
            x='time_window',
            y='tasks_completed',
            ...
            linewidth=2.5,
            markersize=6,
            errorbar='sd',  # Muestra desviación estándar como área sombreada si existe varianza
            ax=ax
        )

        # Controlar de forma dinámica las etiquetas del eje X continuo
        time_windows = sorted(perf['time_window'].unique())
        step = max(1, len(time_windows) // 12)  # Muestra un máximo aproximado de 12 etiquetas para limpieza visual
    ...
    def plot_f_measure_boxplot(self, window_sec: int = 300, max_time_sec: int = 10000, save_path: Optional[str] = None) -> None:
        ...
        # Líneas de tendencia para el análisis del F-measure
...
def main():
    ...
    try:
        ...
        # 2. Curva de Tendencia de Rendimiento (Líneas continuas reemplazando Boxplots)
