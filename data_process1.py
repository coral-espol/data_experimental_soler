...
class RobotDataProcessor:
    ...
    def plot_performance_boxplot(self, window_sec: int = 300, max_time_sec: int = 10000, save_path: Optional[str] = None) -> None:
        if self.raw_data is None or self.raw_data.empty: return
        logger.info("Generating performance boxplot with trend lines for cases")
        
        # Calcular el máximo global real para mantener la saturación estática del eje Y
        ...
        # Controlar la saturación de etiquetas del eje X con formato decimal dinámico
...
def main():
    ...
    # NUEVO: PARÁMETRO DE FILTRO DE ESTRATEGIA
    # Opciones válidas: 'both', 'greedy', 'selective'
    TARGET_STRATEGY = 'greedy'  # Cambia a 'selective' o 'both' según lo que quieras analizar
    
    # Crear un directorio de salida dinámico para no sobreescribir imágenes
    ...
    try:
        ...
        # APLICAR FILTRO DE ESTRATEGIA AL DATASET CRUDO
        ...
        if raw_data.empty:
            logger.error("No hay datos disponibles tras aplicar el filtro de estrategia. Verifica tus archivos CSV.")
            return
